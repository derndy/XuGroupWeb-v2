...
This repository is `derndy/XuGroupWeb-v2`; the original `derndy/XuGroupWeb` remains separate. PR #24 has merged. The current JACS figure and status-record batch continues on `content/jacs-visual-site-status`. Read [`project-state.md`](project-state.md) for the active PR, exact source version, image coverage, completed checks and prioritized open work. The complete older state record is preserved in [`project-history-through-pr24-2026-09-06.md`](project-history-through-pr24-2026-09-06.md).

Current visual coverage includes seven conceptual illustrations, three wide/mobile pillar frameworks, two worked/conceptual examples, four selected-paper figures, 23 Gallery photographs and the existing member/group photos. Preserve the original research identity, formal pillars, five selected papers and full Vision at the end of Join / Collaborate. Source and figure-reuse records live in `data/research_assets.yml`, `data/paper_visuals.yml` and their linked review notes. Browser layout testing remains open; successful builds and asset checks are not browser verification.

Run `python -B scripts/audit-site-images.py /absolute/build` after image changes. Add `--json` to inspect the page-by-page empty-description queue. The script checks local image and responsive-picture references; external URLs are reported without being fetched.
